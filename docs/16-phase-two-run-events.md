...
| `ingest_phase_one_report` | `run.completed` 或 `run.failed` |
...
- worker 执行 queued run 后生成 started/completed/artifact timeline。
...
1. API 创建 run 后只返回 queued。
2. 后台 worker 独立消费 queued run。
3. Worker 每个步骤写 `RunEvent`。
4. `/events/stream` 从轮询式输出升级为长连接实时推送。
5. 前端 timeline 可以无刷新显示 run 进度。
